  ...
  return (
    <div className="detail-page">
      ...
      <div className="detail-body container" style={{ paddingTop: "140px" }}>
        <Link to="/inspirations" className="detail-back-link" style={{ marginBottom: "24px", display: "inline-block" }}>
          ← 返回灵感列表
        </Link>
        ...
        <h1 className="detail-title" style={{ fontSize: "clamp(2rem,4vw,3rem)", marginBottom: "8px" }}>
          {item.title}
        </h1>

        {item.subtitle && (
          <p className="detail-subtitle" style={{ marginBottom: "48px" }}>
            {item.subtitle}
          </p>
        )}
        ...
        {item.link && (
          <div className="inspiration-footer-actions">
            <button className="inspiration-link-btn" onClick={openLink}>
              <span>🔗</span> 查看灵感来源
            </button>
          </div>
        )}
      ...
      <div className="container">
        <div className="detail-pager">
          {prevItem ? (
            <Link to={`/inspiration/${prevItem.id}`} className="dp-link dp-prev">
              <span className="dp-arrow">←</span>
              <span className="dp-body">
                <span className="dp-label">上一篇</span>
                <span className="dp-title">{prevItem.title}</span>
              </span>
            </Link>
          ) : (
          ...
          {nextItem ? (
            <Link to={`/inspiration/${nextItem.id}`} className="dp-link dp-next">
              <span className="dp-body" style={{ textAlign: "right" }}>
                <span className="dp-label">下一篇</span>
                <span className="dp-title">{nextItem.title}</span>
              </span>
              <span className="dp-arrow">→</span>
            </Link>
          ) : (
