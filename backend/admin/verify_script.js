...
function renderSite() {
  ...
  document.getElementById("content").innerHTML =
    ...
    '<div class="row"><div class="form-group"><label>导航「联系我」按钮文字</label><input id="site-contact-btn" value="' + esc(data.site.contactBtnText || "联系我") + '" /></div><div class="form-group"><label>Logo 文字（没上传图片时显示）</label><input id="site-logo" value="' + esc(data.site.logo) + '" /></div></div>' +
  ...
  var lu = document.getElementById("logo-upload");
  if (lu) lu.addEventListener("change", function(e) {
    ...
    xhr.onload = function() {
      if (xhr.status === 200) {
        data.site.logoUrl = JSON.parse(xhr.responseText).url;
        toast("Logo 已上传");
      } else { toast("上传失败"); }
    };
...
async function saveSite() {
  ...
  await fetchAPI("/api/site", { method: "PUT", body: JSON.stringify(body) });
  toast("已保存");
}
...
function editStrength(id) {
  ...
  openModal(
    ...
    '<div class="row"><div class="form-group"><label>中文标题</label><input id="es-titlecn" value="' + esc(s.titleCN) + '" /></div><div class="form-group"><label>标签（逗号分隔）</label><input id="es-tags" value="' + esc((s.tags || []).join(", ")) + '" /></div></div>' +
...
function renderContact() {
  ...
  var c = data.contact;
  var qrImg = c.qrUrl ? '<img src="' + c.qrUrl + '" style="max-width:100px;max-height:100px;display:block;margin-top:8px;border-radius:8px" />' : "";
  document.getElementById("content").innerHTML =
    '<div class="row"><div class="form-group"><label>Email</label><input id="c-email" value="' + esc(c.email) + '" /></div><div class="form-group"><label>微信 ID</label><input id="c-wechat" value="' + esc(c.wechat) + '" /></div></div>' +
    ...
    '<div class="form-group"><label>二维码图片（推荐: 200x200px 正方形, PNG/WebP）</label><input type="file" id="qr-upload" accept="image/png,image/webp,image/jpeg,image/svg+xml" style="padding:8px;background:#181818;border:1px solid rgba(255,255,255,0.08);border-radius:8px;color:#f5f5f5;width:100%" />' + qrImg + '</div>' +
    '<button class="btn" onclick="saveContact()">保存设置</button>';
  var qu = document.getElementById("qr-upload");
  if (qu) qu.addEventListener("change", function(e) {
    var file = e.target.files[0];
    if (!file) return;
    var fd = new FormData(); fd.append("file", file);
    var xhr = new XMLHttpRequest();
    ...
    xhr.onload = function() {
      if (xhr.status === 200) {
        data.contact.qrUrl = JSON.parse(xhr.responseText).url;
        toast("二维码已上传");
      } else { toast("上传失败"); }
    };
    xhr.onerror = function() { toast("上传失败"); };
