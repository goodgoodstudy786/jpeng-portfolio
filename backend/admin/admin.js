...
function editProject(id) {
  ...
  openModal(
    ...
    '<div class="form-group"><label>章节（JSSON格式）?/label><textarea id="ep-sections" style="min-height:100px">' + esc(JSON.stringify(p.sections || [], null, 2)) + '</textarea></div>' +
...
function renderStrengths() {
  document.getElementById("section-title").textContent = "涓优势";
  var h = '<div class="card-list">';
...
function editStrength(id) {
  ...
  openModal(
    ...
    '<div class="row"><div class="form-group"><label>涓枃标题</label><input id="es-titlecn" value="' + esc(s.titleCN) + '" /></div><div class="form-group"><label>鏍囩锛堥€标签（逗号分隔）?/label><input id="es-tags" value="' + esc((s.tags || []).join(", ")) + '" /></div></div>' +
